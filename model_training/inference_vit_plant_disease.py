...
def load_model_and_processor(model_dir, device):
    """Load the fine-tuned ViT model and image processor."""
    try:
...
def main():
    ...
    if len(sys.argv) > 1:
        ...
        print(f"[INFO] No image path provided, using default: {image_path}")

    # Wait for image to exist (useful when called from espcode.py)
    while not os.path.exists(image_path):
        print(f"[waiting] {os.path.basename(image_path)} not found yet...")
        time.sleep(1)
    ...
    if os.path.exists(DATASET_DIR):
        ...
        print("[WARNING] Using generic class labels")
        # Fallback if dataset folder is missing
        id2label = {str(i): f"Class_{i}" for i in range(38)}  # PlantVillage has 38 classes
